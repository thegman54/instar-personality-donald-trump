-- Personality Donald Trump: Reactions table
-- Trigger → response pattern mappings for contextual behavior.

CREATE TABLE IF NOT EXISTS personality_donald_trump_reactions (
...
CREATE INDEX IF NOT EXISTS idx_pdt_reactions_profile
    ON personality_donald_trump_reactions(profile_slug);
CREATE INDEX IF NOT EXISTS idx_pdt_reactions_trigger
    ON personality_donald_trump_reactions(profile_slug, trigger_type);
CREATE INDEX IF NOT EXISTS idx_pdt_reactions_intensity
    ON personality_donald_trump_reactions(profile_slug, intensity);
CREATE INDEX IF NOT EXISTS idx_pdt_reactions_tags
    ON personality_donald_trump_reactions USING GIN(tags);
