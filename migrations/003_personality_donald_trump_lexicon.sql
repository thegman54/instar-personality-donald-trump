-- Personality Donald Trump: Lexicon table
-- Vocabulary fingerprint — words/phrases to favor, avoid, or use situationally.

CREATE TABLE IF NOT EXISTS personality_donald_trump_lexicon (
...
CREATE INDEX IF NOT EXISTS idx_pdt_lexicon_profile
    ON personality_donald_trump_lexicon(profile_slug);
CREATE INDEX IF NOT EXISTS idx_pdt_lexicon_type
    ON personality_donald_trump_lexicon(profile_slug, usage_type);
CREATE INDEX IF NOT EXISTS idx_pdt_lexicon_freq
    ON personality_donald_trump_lexicon(profile_slug, frequency);
